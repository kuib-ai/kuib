...
Discussions are named, toggleable clusters of parts ([[roadmap/items/R306-discussions-context-control]], [[_archive/protocol-design/decisions#Discussion Clustering]]). The user saves visual selections, includes/excludes whole discussions at runtime, and references them across sessions. This dialog is the management surface — not the inline Context pane list (which is quick-toggle only). Opened with `:discussion` or `gd` from Conversation.
...
① `linked: session-abc` — cross-session `DiscussionLinked` reference ([[roadmap/items/R306-discussions-context-control]] open question: snapshot vs live)
...
① open question from [[roadmap/items/R306-discussions-context-control]] — default included on create vs opt-in; wireframe assumes opt-in unchecked.
