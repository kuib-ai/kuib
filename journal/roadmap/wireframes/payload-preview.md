...
Transparency ([[_archive/vision/decisions#Mechanisms]]): the user sees **exactly** what the next provider call will receive — after branch resolution, discussion toggles, and part exclusions. This is the proof that context control is real, not a filter hidden in the engine. Opened before send (optional gate) or on demand (`gp` / status strip click). Must show token + cache economics, not just pretty text.

Dialog overlays [[roadmap/wireframes/session-layout]]; not a route.
...
Right rail omits cache lines; shows `cache: n/a (openai-compatible)` — per-provider rules ([[_archive/provider-architecture/decisions#Open Questions]]).
