...
① estimates from `TokenUsage` + `ModelRef` on recorded step boundaries — prefix `~` when interrupted ([[domains/core/decisions#^D009]])
...
Edits at or below the `h` gutter row in [[roadmap/wireframes/conversation]] apply immediately — cache-safe per protocol option B.
