...
- **Runtime is fine either way** — Bun resolves `@/` per-file (nearest tsconfig); the break is TypeScript-only. **Superseded (2026-09-22):** **no longer true** — Deno is the runtime and does not apply tsconfig `paths`, so any `@/` or `@pkg/*` alias must resolve through package `exports` (or an import map) to work at runtime. See `journal/features/deno-runtime/plan.md`.
