...
- ~~Extract the transcript fold to a shared package~~ **DONE** — both hosts import `Transcript.foldTranscript` from `@kuib-ai/transcript`; the fold now segments on tool/user boundaries (see [[host-layer]] Turn scheduling).
- Daemon test-suite leaks detached `start.daemon` child processes (`ensure.daemon` spawn-path tests launch real daemons that outlive the run — ~20 had accumulated). Fix: kill the spawned child in test teardown, or give the daemon an idle self-reap like the engine-service.
