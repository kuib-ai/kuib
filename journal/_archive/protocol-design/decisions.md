...
**Superseded (2026-09-22):** "later" arrived — `@kuib-ai/event-log-sqlite` uses **`node:sqlite`** (`DatabaseSync`, compiled into the Deno binary, no `dlopen`), WAL unchanged, transactions via prepared `BEGIN IMMEDIATE`/`COMMIT`/`ROLLBACK`. Deno ≥ 2.9.7 rejects symlinked DB paths. See `journal/features/deno-runtime/plan.md` D003, G003.
