...
3. **The binary `apps/kuib`** — multi-role entry (`kuib`/`daemon`/`serve`/`up`); wire env→provider→engine→daemon; one `bun build --compile` target (**Superseded (2026-09-22):** runtime is Deno, compile target becomes `deno compile`; `journal/features/deno-runtime/plan.md`). First runnable end-to-end.
4. **Host (TUI) rewrite** — OpenTUI+Solid (**Superseded (2026-09-22):** kuib's own terminal UI library, `journal/features/deno-runtime/plan.md` D005); subscribe to committed log, render, submit `UserMessageSubmitted`; fix host→engine boundary; nvim-flavored. See [[host-layer]].
5. **Context control** — transparency (see-before-send), per-part exclusion, discussion clustering ([[discussions-ux]], [[vision]]).
6. **Packaging** — per-platform `bun build --compile` (**Superseded (2026-09-22):** `deno compile`, revisited once a host has a UI); `kuib service install` (**join** mode only); `.env`/`~/.kuib/config`.
