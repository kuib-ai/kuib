...
- **Opt-in**: `startTelemetry` is a no-op returning `false` when `endpoint` is unset, so normal runs are unaffected. Called once at the start of the `serve` process in `host-tui` and `host-web` (host-web since deleted) (the processes that run the model).

(**Superseded (2026-09-22):** the runtime is now Deno; this end-to-end span check has **not** been re-run on Deno — Deno also ships built-in OpenTelemetry (`OTEL_DENO=true`) worth evaluating. `journal/features/deno-runtime/plan.md`) Verified end-to-end on **Bun + AI SDK v7**: a spike span landed in Phoenix (`kuib-spike` project, traceCount 1), confirming the one runtime unknown (Bun/v7/ESM/Node-22).
