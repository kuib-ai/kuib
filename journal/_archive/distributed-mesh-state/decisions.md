...
**Deferred → next:** Headscale-backed `DiscoveryPort` (dynamic registry replacing the static TOML), CRDT config/key `SyncState`, and the ~~`@opentui/keymap`~~ device switcher (**Superseded (2026-09-22):** keymap is owned by kuib's own terminal UI library, `journal/features/deno-runtime/plan.md` D005).
