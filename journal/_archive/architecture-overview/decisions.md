...
- **Superseded (2026-09-22): Deno is the runtime, used purely as a runtime** — pnpm resolves packages, Nx runs tasks, `tsgo` type-checks against Deno declarations generated by Nx (`@kuib-ai/deno-types`); no `deno.json`. OpenTUI, Solid front ends, and Bun were removed. See `journal/features/deno-runtime/plan.md` (D002, D006).
...
- ~~**Distribution: per-platform self-contained binaries via `bun build --compile`**~~ **Superseded (2026-09-22):** distribution is revisited once a host has a UI again (`deno compile` is the candidate); see `journal/features/deno-runtime/plan.md`. — bundles the Bun runtime + JS + the native OpenTUI `.so/.dylib` into one executable; users install nothing.
- ~~**Toolchain provisioning (2026-07-02): pnpm stays the package manager; bun is provisioned BY pnpm.**~~ **Superseded (2026-09-22):** Bun is gone; **Deno is provisioned BY pnpm** the same way — `deno` is a root devDependency, catalog-pinned (`2.9.6`, newest on npm) with `allowBuilds: deno: true`, so `pnpm install` puts it in `node_modules/.bin`; no system install needed. See `journal/features/deno-runtime/plan.md` D007. `bun` (stable, catalog-pinned `^1.3.14`) is a root devDependency with `allowBuilds: bun: true`, so `git clone && pnpm install` fetches the platform binary into `node_modules/.bin` on any device (proven on minerva/darwin) — no system bun install, no version drift across the mesh. `pnpm run` scripts resolve that pinned bun via PATH. Trade-off accepted: npm ships stable only (no canary).
...
- ~~**TUI framework → OpenTUI + Solid reconciler**~~ **Superseded (2026-09-22):** kuib builds its own terminal UI library (pure TS, Solid as the reactive core via `solid-js/universal`, owned renderer/layout); see `journal/features/deno-runtime/plan.md` D005. (Zig native core, fine-grained reactivity = render-only-on-change, flicker-free double-buffered cell rendering). See [[host-layer]], [[protocol-design]].
- **Embed nvim subprocess vs grid → deferred to v1.x.** v1 is nvim-_flavored_ (modal keymap/leader — originally OpenTUI's; **Superseded (2026-09-22):** kuib's own library owns the keymap, `journal/features/deno-runtime/plan.md` D005), not real embedded nvim. See [[nvim-integration]] (now `stale`).
