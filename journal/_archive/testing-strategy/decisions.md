---
title: Testing Strategy — deno test, colocated, hermetic
type: implementation
layer: process
status: decided
created: 2026-07-02
tags: [testing, deno-test, bun-test, coverage, hermeticity, conventions]
...
- **eslint-plugin rules** are tested with `@typescript-eslint/rule-tester`'s `RuleTester` (statics wired to `@std/testing/bdd`'s `describe/it/afterAll` — originally `bun:test`'s, Superseded (2026-09-22): see `journal/features/deno-runtime/plan.md`) — eslint-core's `Linter`/`RuleTester` types are nominally incompatible with `RuleCreator` rules and would force banned casts.
...
Agents/automation that write tests must verify with **all gates, not just the runner**: ~~`bun test packages apps`, `pnpm -r --filter '!@kuib-ai/tsconfig' exec tsc --noEmit`, `pnpm exec eslint .`~~ (**Superseded (2026-09-22):** `corepack pnpm nx run-many -t test` + `pnpm run check` — tsgo typecheck, eslint, prettier; `journal/features/deno-runtime/plan.md`), madge. The 2026-07-02 sweep proved runner-only verification lies: 46 "green" tests hid 22 lint errors, 8 typecheck failures, and one cross-file mock leak.
