...
layer: experience
created: 2026-06-18
tags: [host, deno, solid, nvim, ui, panes, adapter, opentui, bun]
depends-on:
  [
...
One `bun build --compile` artifact contains Service + Daemon + TUI; argv selects role (**Superseded (2026-09-22):** the runtime is Deno and the artifact becomes `deno compile`; the multi-role argv design stands — `journal/features/deno-runtime/plan.md`):
...
**Superseded (2026-09-22):** both columns now run on Deno; `event-log-sqlite` uses `node:sqlite`. The split still stands on IO-separation grounds (`journal/features/deno-runtime/plan.md`).
...
- ~~**Known gap:** host-web bypasses the engine-service~~ (**Superseded (2026-09-22):** moot — host-web was deleted, `journal/features/deno-runtime/plan.md` D002.) (`void runAgent` per POST) so it still allows concurrent turns — resolves with the shared-host-bootstrap follow-up (see the host-duplication smell in [[testing-strategy]]'s sweep).
- (**Superseded (2026-09-22):** OpenTUI removed; kuib's own library decides mouse tracking.) TUI mouse-selection note: opentui enables mouse tracking, so terminal text selection needs Shift+drag; in-app copy-on-select is part of the deferred `@opentui/keymap` pass.
...
**Superseded (2026-09-22):** `apps/host-web` (and its Vite/Solid-DOM client) was deleted; this section records the design for when a web host returns (`journal/features/deno-runtime/plan.md` D002).
...
Adopted from the opencode study (their `packages/tui` — same `@opentui/solid` stack, fully grown). **Superseded (2026-09-22):** the provider-stack / route-store / screens layering still applies, but on kuib's own terminal UI library with Solid as the reactive core, not `@opentui/solid` (`journal/features/deno-runtime/plan.md` D005). Three layers; **no router library**:
...
- **Tier 1 (now):** `dev:watch` = `bun --watch --no-clear-screen run src/index.tsx` (**Superseded (2026-09-22):** `deno run -A --watch src/index.ts`). Hard restart on edit → reattach to `engine.sock` → refold from the log. `--no-clear-screen` is required: Bun's clear escapes corrupt alternate-screen TUIs.
...
- **Tier 2 (experimental, deferred):** ~~`bun --hot` same-PID re-eval + opentui's `singleton()` renderer reuse~~ (**Superseded (2026-09-22):** Bun and OpenTUI removed; re-evaluate against Deno + kuib's own renderer). Possible but self-plumbed: no Solid refresh exists (solid-refresh is bundler-only; only React got in-place refresh, opentui PR #446), re-creating the renderer throws stdin-exclusivity, and listeners/timers leak without manual disposal. opentui maintainers recommend `--watch`.
