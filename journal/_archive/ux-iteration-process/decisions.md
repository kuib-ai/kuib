...
2. **Build** — implement against the wireframe with OpenTUI + Solid (**Superseded (2026-09-22):** kuib's own terminal UI library, `journal/features/deno-runtime/plan.md` D005).
...
5. **Lock in** — frame snapshots become regression tests (`bun test` `toMatchSnapshot`; **Superseded (2026-09-22):** snapshot mechanism to be rebuilt on `deno test` + `@std/testing/snapshot` with the new library).

## Tooling decisions

- **Superseded (2026-09-22):** the skill no longer loads an `opentui` skill (deleted); frames are sketched in cell-grid terms. **`/wireframe` skill** (`.claude/skills/wireframe/`): the entry point for designing/editing any screen wireframe. Loads this entry's conventions, the `opentui` skill, and the current wireframe/codebase state before drawing; validates via the index compiler after.
...
- **Superseded (2026-09-22):** `testRender` + Bun snapshots were removed with OpenTUI. **Snapshot tests**: `testRender` char frames via Bun snapshots — the OpenTUI analog of Textual's `pytest-textual-snapshot`.
- **Superseded (2026-09-22):** OpenTUI-native tooling (anscribe) no longer applies. **Deferred/optional**: anscribe (point-at-renderable → agent capture, OpenTUI-native), tuistory/pilotty (PTY driver for agent-run e2e), VHS (scripted GIFs for design review).
