...
> [!sources]- structure · verified 2026-09-25
...
> - `.agents/mcp_config.json` · #12aa8a46
> - `.mcp.json` › "mcpServers"
...
> [!sources]- behaviour · verified 2026-09-25
...
> - `.agents/generated.lock.json` · #bef0ad2a
...
Other scripts live in `scripts/`; any package they import is a root devDependency. ^workspace-tools
...
`scripts/roadmap.graph.ts` reads the roadmap as a graph from the mermaid block of the generated
`roadmap/ROADMAP.md`: each item's ID, title and horizon (the subgraph it sits in, `closed`
outside any), and each edge's kind (`depends` for `-->`, otherwise the label of a dotted link:
`converges`, `split` or `absorbed`) and whether it is directed. `neighbourhood` keeps the
selected items, every item one edge away and the edges among them, and throws on an ID the graph
does not contain. `edgeArrow` writes `depends` edges as `-->` and the others as labelled dotted
links, directed (`-.->|kind|`) or not (`-.-|kind|`). ^roadmap-graph

> [!sources]- behaviour · verified 2026-09-25
> - `scripts/roadmap.graph.ts` › `readRoadmapGraph` · #29801dbe
> - `scripts/roadmap.graph.ts` › `neighbourhood` · #e4b4fd47
> - `scripts/roadmap.graph.ts` › `edgeArrow` · #c05f3b3d
> - why: [[domains/infra/decisions#^D031]]

`pnpm roadmap [R###…]` (`scripts/roadmap.ts`) draws the whole graph, or the neighbourhood of
the given items (case-insensitive), as box art with `beautiful-mermaid`'s ASCII renderer: a flat
`flowchart LR` without horizon subgraphs, each box labelled `R### · <horizon>` above its title
wrapped at 28 columns, `[` and `]` replaced by `⟦` and `⟧` (the renderer ends a label at a
bracket), and no colour codes. On a terminal the drawing is paged through `less -S`, which
scrolls sideways; otherwise it is written to stdout. ^roadmap-terminal

> [!sources]- behaviour · verified 2026-09-25
> - `scripts/roadmap.ts` › `toMermaid` · #ef75eff6
> - `scripts/roadmap.ts` › `if (process.stdout.isTTY === true) {` · #d99687a7
> - `package.json` › ""roadmap": "deno run -A scripts/roadmap.ts","
> - `package.json` › ""beautiful-mermaid": "catalog:","
> - why: [[domains/infra/decisions#^D031]]

`pnpm roadmap:serve [--port <n>] [--host <address>]` (`scripts/roadmap.serve.ts`, port 4700
by default) serves the roadmap over HTTP. Without `--host` it listens on `127.0.0.1` and, when
`tailscale status --json` reports one, on the node's tailnet IPv4 address, printing each URL by
its MagicDNS name; the address is looked up at start and never written down. `/` is the whole
graph and `/R###` the neighbourhood of one item (404 for an item not on the graph);
`?group=horizon` draws horizons as subgraphs. `/mermaid/…` and `/layout-elk/…` serve files from
the `dist` folders of the root devDependencies `mermaid` and `@mermaid-js/layout-elk`, refusing
paths that resolve outside them; anything else is a 404. `ROADMAP.md` is re-read on every page
request. ^roadmap-serve

> [!sources]- behaviour · verified 2026-09-25
> - `scripts/roadmap.serve.ts` › `tailnetListener` · #cda3616d
> - `scripts/roadmap.serve.ts` › `serveAsset` · #3212c3b2
> - `scripts/roadmap.serve.ts` › `handle` · #49be5ffa
> - `scripts/roadmap.serve.ts` › `listeners` · #7b97cddc
> - `package.json` › ""roadmap:serve": "deno run -A scripts/roadmap.serve.ts","
> - `package.json` › ""mermaid": "catalog:","
> - `package.json` › ""@mermaid-js/layout-elk": "catalog:","
> - why: [[domains/infra/decisions#^D031]]

The page lays the graph out in the browser with Mermaid and its ELK layout: `flowchart TB`, one
colour per horizon (a `classDef` each, repeated as a legend in the top bar), Coffman–Graham
layering with at most 10 items per layer, and, in the flat view, items without edges chained by
invisible `~~~` links into columns about √n tall so they do not stretch a single layer. The SVG
sits on a full-window canvas: dragging or scrolling pans, pinching or ⌘/ctrl-scrolling zooms
around the pointer, the `−`, `+` and `fit` buttons and the `+`, `-`, `f` and `0` keys zoom, fit
never enlarges past 1.5×, and clicking an item opens its neighbourhood. While the layout runs the
canvas counts `rendering… Ns`; a failure replaces it with the error and its stack. ^roadmap-canvas

> [!sources]- behaviour · verified 2026-09-25
> - `scripts/roadmap.serve.ts` › `HORIZON_STYLES` · #cf64f80a
> - `scripts/roadmap.serve.ts` › `toMermaid` · #04c6b578
> - `scripts/roadmap.serve.ts` › `page` · #7cb1b9a3
> - why: [[domains/infra/decisions#^D031]]
