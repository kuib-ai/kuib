...
`engine-service`, `event-log-sqlite`, `protocol`, `std` and `telemetry`. ^C001

> [!sources]- C001 · structure · verified 9374dfb
> - `apps/host-tui/package.json` › "Terminal host for Kuib AI"
> - `apps/host-tui/src/run/index.ts` › `run` · #10566b1f
> - `apps/host-tui/src/index.ts` › `main` · #765e91db
...
type-checking is `tsgo --noEmit` against the shared base tsconfig. ^C002
...
parsed arguments and config file path. ^C003
...
invocation mints a fresh random `DeviceID` and hands it to the command. ^C004
...
the logger exists, echoed to stderr, and the process exits 1. ^C005
...
parsed but not forwarded to the bootstrap. ^C006

> [!sources]- C006 · behaviour · verified 9374dfb
> - `apps/host-tui/src/index.ts` › `cliSchema` · #7aacbc36
...
entry does not import it and keeps its own inline copy. ^C007

> [!sources]- C007 · structure · verified 9374dfb
> - `apps/host-tui/src/cli/index.ts` › `cli` · #c49e3551
> - `apps/host-tui/src/cli/schema/index.ts` › `schema` · #a225aae2
...
model, provider options and `ModelRef` once for the whole process. ^C008

> [!sources]- C008 · behaviour · verified 9374dfb
> - `apps/host-tui/src/run/serve/index.ts` › `serve` · #200a6edf
...
rejects a node the mesh config does not list. ^C010

> [!sources]- C010 · behaviour · verified 9374dfb
> - `apps/host-tui/src/resolve.daemon.client/index.ts` › `resolveDaemonClient` · #b280cc94
...
field on every record. ^C011

> [!sources]- C011 · behaviour · verified 9374dfb
> - `apps/host-tui/src/log/index.ts` › `createLog` · #55febeb0
...
reachable as `@kuib-ai/cli/<section>`. ^C012

> [!sources]- C012 · structure · verified 9374dfb
> - `packages/cli/src/index.ts` › `Cli` · #a887d21b
...
> - `packages/cli/src/parse.cli/index.ts` › `parseCli` · #895ab25f
...
options, the description and any default. ^C014

> [!sources]- C014 · behaviour · verified 9374dfb
> - `packages/cli/src/print.help/index.ts` › `printHelp` · #74cb8319
